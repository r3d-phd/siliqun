...
class SiliQunEnv(BASE_CLASS):
    ...
    target_state : str or MPS
        Target state to prepare. Options: "ghz", "bell", "w", "cluster_linear", "dicke_k{n}",
    ...
    def _build_target(self, target_state):
        ...
        if self.sim_mode == "sv":
            ...
            elif target_state == "cluster_linear":
                self._target_state = MPS.cluster_linear_state(self.n_qubits)
            elif target_state is not None and str(target_state).startswith("dicke_k"):
                # Format: "dicke_k{n}" e.g. "dicke_k2" means k=2
                try:
                    k = int(str(target_state).split("_k")[1])
                except (IndexError, ValueError):
                    raise ValueError(
                        f"Invalid dicke target format '{target_state}'. "
    ...
    def _build_sv_target(self, target_state) -> np.ndarray:
        ...
        if isinstance(target_state, np.ndarray):
            ...
            # Build cluster linear state via MPS then convert to dense
            mps_cl = MPS.cluster_linear_state(n)
            sv = mps_cl.to_dense().flatten()
        elif target_state is not None and str(target_state).startswith("dicke_k"):
            try:
                k = int(str(target_state).split("_k")[1])
            except (IndexError, ValueError):
                raise ValueError(f"Invalid dicke target format '{target_state}'.")
            mps_dk = MPS.dicke_state(n, k)
            sv = mps_dk.to_dense().flatten()
        elif target_state == "random":
...
def make_siliqun_env(
    ...
    target : str
        Target state: "bell", "ghz", "w", "cluster_linear", "dicke_k{n}", or "random".
    sim_mode : str
