...
| `shor_n15_order4_work_orbit_transition_native` | A two-qubit work-orbit transition. q0 and q1 encode `1,2,4,8` as `00,01,10,11`; a CNOT-equivalent q0→q1 followed by an X-equivalent q0 advances that restricted encoding modulo four, corresponding to multiplication by 2 on this orbit modulo 15. | Tests one native two-qubit work-orbit transition only; it has no control register, phase estimation, inverse QFT, measurement, classical post-processing, order, factor, or full Shor execution. |

The third representation was reduced from a pre-execution three-qubit proposal after the first bounded source-export attempt correctly refused it: `simos-nominal-literature-v1` declares only two qubits. The replacement is a separate, newly frozen two-qubit representation. It is an uncontrolled restricted work-orbit transition, not reversible modular multiplication over every residue class modulo 15. No result from B3 may be called a Shor run, order-finding outcome, or factorization result.
...
An X-equivalent is `rx(π)` up to global phase. A Hadamard-equivalent is the ordered native sequence `rz(π)` then `ry(π/2)`, also up to global phase. A CNOT-equivalent is this Hadamard-equivalent on the target, followed by `cz(control,target)`, followed by the same Hadamard-equivalent. The two-qubit work-orbit transition applies this CNOT-equivalent q0→q1, then the X-equivalent on q0.
...
The agreement mechanism is deliberately narrow and explicit. For each `representation_id`, a later evaluator must first require that `source_row.circuit_specification_sha256` equals the independently generated static pin for that same identifier. It then receives the byte-pinned source receipt containing that exact gate list and `source_row.siliqun_ideal_probability_sha256`, applies the listed gates to a fresh local state using independently written ideal matrices under the same least-significant-bit basis convention, creates a second transient probability list, and applies the same rounding, serialization, and SHA-256 rule. Its retained `digest_agrees` boolean must be exactly `evaluator_ideal_probability_sha256 == source_row.siliqun_ideal_probability_sha256` for that same statically pinned row. Agreement of that boolean for every fixed row, together with the source and evaluator audits, is the only basis for a B3 cross-software convention statement.

## Independent evaluator seeding and ordered evidence chain

Before the source export runs, independently authored SiVQD code must construct the three full native-gate specifications without importing SiliQun or reading the SiliQun static receipt. It must write a separately versioned SiVQD static receipt and compare only its independently computed specification hashes and identities to the frozen B3 static receipt. Its evaluator catalogue may then record the agreed three representation identifiers, static hashes, native gate set, LSB/canonicalization rules, retained-field requirements, forbidden-field requirements, and claim ceiling. It must not contain a source probability digest, source receipt byte hash, simulation output, or source-export result.
