...
For each circuit, the exporter must create a genuine `TranslationRequest` and `TranslationReceipt` with `simos_nominal_profile()`, run `StateVectorSimulator`, and retain only the circuit specification, source identities, profile status, circuit digest, translation-manifest digest, pulse-schedule digest, operation categories and target arities, and an SHA-256 digest of a canonical ideal probability vector. The canonical list is indexed in ascending computational-basis integer order, with qubit 0 as the least-significant bit. Each probability is converted with `float()` and processed by Python `round(value, 15)` (the runtime's IEEE 754 round-half-to-even behavior); the list is encoded using `json.dumps(values, separators=(",", ":"), ensure_ascii=True)` and SHA-256 is taken over its UTF-8 bytes. It must discard the state vector and probability vector before writing the receipt.
